import {
  ...
  toRef as toRef$1,
  toRefs as toRefs$1,
  toValue as toValue$1,
  unref,
  watch,
  watchEffect,
} from "./vue.runtime.esm-bundler-DMz-3vGB.js";
...
function reactiveOmit(obj, ...keys$1) {
  ...
  return reactiveComputed(() =>
    typeof predicate === "function"
      ? Object.fromEntries(
          Object.entries(toRefs$1(obj)).filter(
            ([k, v]) => !predicate(toValue$1(v), k),
          ),
        )
      : Object.fromEntries(
          Object.entries(toRefs$1(obj)).filter((e) => !flatKeys.includes(e[0])),
        ),
  );
}
...
function reactivePick(obj, ...keys$1) {
  ...
  return reactiveComputed(() =>
    typeof predicate === "function"
      ? Object.fromEntries(
          Object.entries(toRefs$1(obj)).filter(([k, v]) =>
            predicate(toValue$1(v), k),
          ),
        )
...
function useArrayDifference(...args) {
  ...
  const diff1 = computed(() =>
    toValue$1(list).filter(
      (x) => toValue$1(values).findIndex((y) => compareFn(x, y)) === -1,
    ),
  );
  if (symmetric) {
    const diff2 = computed(() =>
      toValue$1(values).filter(
        (x) => toValue$1(list).findIndex((y) => compareFn(x, y)) === -1,
      ),
    );
    return computed(() =>
      symmetric ? [...toValue$1(diff1), ...toValue$1(diff2)] : toValue$1(diff1),
    );
  } else return diff1;
}
function useArrayEvery(list, fn) {
  return computed(() =>
    toValue$1(list).every((element, index, array) =>
      fn(toValue$1(element), index, array),
    ),
  );
}
...
function useArrayFind(list, fn) {
  return computed(() =>
    toValue$1(
      toValue$1(list).find((element, index, array) =>
        fn(toValue$1(element), index, array),
      ),
    ),
  );
...
function useArrayFindIndex(list, fn) {
  return computed(() =>
    toValue$1(list).findIndex((element, index, array) =>
      fn(toValue$1(element), index, array),
    ),
  );
}
  ...
  return computed(() =>
    toValue$1(
      !Array.prototype.findLast
        ? findLast(toValue$1(list), (element, index, array) =>
            fn(toValue$1(element), index, array),
          )
        : toValue$1(list).findLast((element, index, array) =>
            fn(toValue$1(element), index, array),
          ),
    ),
  );
...
function useArrayIncludes(...args) {
  ...
  comparator =
    comparator != null
      ? comparator
      : (element, value2) => element === toValue$1(value2);
  return computed(() =>
    toValue$1(list)
      .slice(formIndex)
...
function useArrayReduce(list, reducer, ...args) {
  const reduceCallback = (sum, value, index) =>
    reducer(toValue$1(sum), toValue$1(value), index);
  return computed(() => {
...
function useArraySome(list, fn) {
  return computed(() =>
    toValue$1(list).some((element, index, array) =>
      fn(toValue$1(element), index, array),
    ),
  );
}
...
function formatDate(date, formatStr, options = {}) {
  ...
  const matches = {
    ...
    MMM: () =>
      date.toLocaleDateString(toValue$1(options.locales), { month: "short" }),
    MMMM: () =>
      date.toLocaleDateString(toValue$1(options.locales), { month: "long" }),
    D: () => String(days),
    ...
    d: () => day,
    dd: () =>
      date.toLocaleDateString(toValue$1(options.locales), {
        weekday: "narrow",
      }),
    ddd: () =>
      date.toLocaleDateString(toValue$1(options.locales), { weekday: "short" }),
    dddd: () =>
      date.toLocaleDateString(toValue$1(options.locales), { weekday: "long" }),
...
function useDateFormat(date, formatStr = "HH:mm:ss", options = {}) {
  return computed(() =>
    formatDate(normalizeDate(toValue$1(date)), toValue$1(formatStr), options),
  );
}
...
function onClickOutside(target, handler, options = {}) {
  ...
  if (isIOS && !_iOSWorkaround) {
    ...
    useEventListener(
      window$1.document.documentElement,
      "click",
      noop,
      listenerOptions,
    );
  ...
  const shouldIgnore = (event) => {
    return toValue$1(ignore).some((target2) => {
      if (typeof target2 === "string")
        return Array.from(window$1.document.querySelectorAll(target2)).some(
      ...
      useEventListener(
        ...
        (event) => {
          setTimeout(() => {
            ...
            const el = unrefElement(target);
            if (
              ((_a = window$1.document.activeElement) == null
                ? void 0
                : _a.tagName) === "IFRAME" &&
              !(el == null
                ? void 0
...
function useMutationObserver(target, callback, options = {}) {
  ...
  const isSupported = useSupported(
    () => window$1 && "MutationObserver" in window$1,
  );
  const cleanup = () => {
    if (observer) {
      observer.disconnect();
...
function onElementRemoval(target, callback, options = {}) {
  const {
    window: window$1 = defaultWindow,
    document: document$1 = window$1 == null ? void 0 : window$1.document,
    flush = "sync",
  } = options;
...
function useActiveElement(options = {}) {
  ...
  if (triggerOnRemoval)
    onElementRemoval(activeElement, trigger, { document: document$1 });
  trigger();
...
function useBattery(options = {}) {
  ...
  const isSupported = useSupported(
    () =>
      navigator$1 &&
      "getBattery" in navigator$1 &&
      typeof navigator$1.getBattery === "function",
  );
...
function useBluetooth(options) {
  ...
  const isSupported = useSupported(
    () => navigator$1 && "bluetooth" in navigator$1,
  );
  ...
  async function requestDevice() {
    ...
    try {
      device.value = await (navigator$1 == null
        ? void 0
...
function useMediaQuery(query, options = {}) {
  const { window: window$1 = defaultWindow, ssrWidth = useSSRWidth() } =
    options;
  const isSupported = useSupported(
    () =>
      window$1 &&
      "matchMedia" in window$1 &&
      typeof window$1.matchMedia === "function",
  );
...
function useBroadcastChannel(options) {
  ...
  const isSupported = useSupported(
    () => window$1 && "BroadcastChannel" in window$1,
  );
...
function useBrowserLocation(options = {}) {
  ...
  for (const [key, ref$1] of objectEntries(refs))
    watch(ref$1, (value) => {
      if (
        !(window$1 == null ? void 0 : window$1.location) ||
  ...
  const buildState = (trigger) => {
    ...
    const { state: state2, length } =
      (window$1 == null ? void 0 : window$1.history) || {};
    const { origin } = (window$1 == null ? void 0 : window$1.location) || {};
    for (const key of WRITABLE_PROPERTIES)
      refs[key].value =
        (_a = window$1 == null ? void 0 : window$1.location) == null
          ? void 0
          : _a[key];
    return reactive({
      trigger,
...
function usePermission(permissionDesc, options = {}) {
  const { controls = false, navigator: navigator$1 = defaultNavigator } =
    options;
  const isSupported = useSupported(
    () => navigator$1 && "permissions" in navigator$1,
  );
...
function useClipboard(options = {}) {
  ...
  const isClipboardApiSupported = useSupported(
    () => navigator$1 && "clipboard" in navigator$1,
  );
  ...
  if (window$1 && listenToStorageChanges)
    tryOnMounted(() => {
      if (storage instanceof Storage)
        useEventListener(window$1, "storage", update, { passive: true });
      else
        useEventListener(
          window$1,
    ...
    (selector2, attribute2, value) => {
      const el =
        typeof selector2 === "string"
          ? window$1 == null
            ? void 0
            : window$1.document.querySelector(selector2)
...
function useCountdown(initialCountdown, options) {
  ...
  const reset = (countdown) => {
    var _a2;
    remaining.value =
      (_a2 = toValue$1(countdown)) != null ? _a2 : toValue$1(initialCountdown);
  };
  const stop = () => {
    intervalController.pause();
    reset();
...
function useCssVar(prop, target, options = {}) {
  ...
  const elRef = computed(() => {
    ...
    return (
      unrefElement(target) ||
      ((_a = window$1 == null ? void 0 : window$1.document) == null
        ? void 0
        : _a.documentElement)
    );
  });
  function updateCssVar() {
    ...
    if (el && window$1 && key)
      variable.value =
        ((_a = window$1.getComputedStyle(el).getPropertyValue(key)) == null
          ? void 0
          : _a.trim()) ||
        variable.value ||
        initialValue;
...
function useDeviceMotion(options = {}) {
  ...
  function init() {
    if (window$1) {
      ...
      useEventListener(window$1, "devicemotion", onDeviceMotion, {
        passive: true,
      });
    }
  }
...
function useDeviceOrientation(options = {}) {
  const { window: window$1 = defaultWindow } = options;
  const isSupported = useSupported(
    () => window$1 && "DeviceOrientationEvent" in window$1,
  );
...
function useDevicePixelRatio(options = {}) {
  ...
  if (window$1)
    stop = watchImmediate(
      query,
      () => (pixelRatio.value = window$1.devicePixelRatio),
    );
  return {
...
function useDevicesList(options = {}) {
  ...
  const isSupported = useSupported(
    () =>
      navigator$1 &&
      navigator$1.mediaDevices &&
      navigator$1.mediaDevices.enumerateDevices,
  );
  const permissionGranted = shallowRef(false);
  let stream;
  ...
  if (isSupported.value) {
    ...
    useEventListener(navigator$1.mediaDevices, "devicechange", update, {
      passive: true,
    });
    update();
  }
...
function useDisplayMedia(options = {}) {
  ...
  const isSupported = useSupported(() => {
    var _a2;
    return (_a2 = navigator$1 == null ? void 0 : navigator$1.mediaDevices) ==
      null
      ? void 0
      : _a2.getDisplayMedia;
  });
...
function useDraggable(target, options = {}) {
  ...
  const position = ref(
    (_a = toValue$1(initialValue)) != null
      ? _a
      : {
          x: 0,
          y: 0,
...
function useResizeObserver(target, callback, options = {}) {
  ...
  let observer;
  const isSupported = useSupported(
    () => window$1 && "ResizeObserver" in window$1,
  );
  const cleanup = () => {
    if (observer) {
      observer.disconnect();
...
function useElementByPoint(options) {
  ...
  const isSupported = useSupported(() => {
    if (toValue$1(multiple))
      return document$1 && "elementsFromPoint" in document$1;
    return document$1 && "elementFromPoint" in document$1;
  });
  const element = shallowRef(null);
  const cb = () => {
    var _a, _b;
    element.value = toValue$1(multiple)
      ? (_a =
          document$1 == null
            ? void 0
            : document$1.elementsFromPoint(toValue$1(x), toValue$1(y))) != null
        ? _a
        : []
      : (_b =
            document$1 == null
              ? void 0
              : document$1.elementFromPoint(toValue$1(x), toValue$1(y))) != null
        ? _b
        : null;
  };
  const controls =
...
function useIntersectionObserver(target, callback, options = {}) {
  ...
  const isSupported = useSupported(
    () => window$1 && "IntersectionObserver" in window$1,
  );
  const targets = computed(() => {
    const _target = toValue$1(target);
...
function useFavicon(newIcon = null, options = {}) {
  ...
  const applyIcon = (icon) => {
    ...
    if (!elements || elements.length === 0) {
      const link =
        document$1 == null ? void 0 : document$1.createElement("link");
      if (link) {
        link.rel = rel;
...
function useFetch(url, ...args) {
  ...
  watch([refetch, toRef(url)], ([refetch2]) => refetch2 && execute(), {
    deep: true,
  });
  const shell = {
...
function useFileSystemAccess(options = {}) {
  ...
  const isSupported = useSupported(
    () =>
      window$1 &&
      "showSaveFilePicker" in window$1 &&
      "showOpenFilePicker" in window$1,
  );
...
function useFullscreen(target, options = {}) {
  ...
  const isCurrentElementFullScreen = () => {
    if (fullscreenElementMethod)
      return (
        (document$1 == null ? void 0 : document$1[fullscreenElementMethod]) ===
        targetRef.value
      );
    return false;
  };
  ...
  async function exit() {
    ...
    if (exitMethod.value)
      if ((document$1 == null ? void 0 : document$1[exitMethod.value]) != null)
...
function useGamepad(options = {}) {
  const { navigator: navigator$1 = defaultNavigator } = options;
  const isSupported = useSupported(
    () => navigator$1 && "getGamepads" in navigator$1,
  );
  const gamepads = ref([]);
  ...
  const updateGamepadState = () => {
    const _gamepads =
      (navigator$1 == null ? void 0 : navigator$1.getGamepads()) || [];
...
function useGeolocation(options = {}) {
  ...
  const isSupported = useSupported(
    () => navigator$1 && "geolocation" in navigator$1,
  );
...
function useIdle(timeout = oneMinute, options = {}) {
  ...
  if (window$1) {
    ...
    for (const event of events$1)
      useEventListener(window$1, event, onEvent, listenerOptions);
    if (listenForVisibilityChange)
      useEventListener(
        document$1,
        "visibilitychange",
        () => {
          if (!document$1.hidden) onEvent();
        },
        listenerOptions,
      );
    reset();
...
function useScroll(element, options = {}) {
  ...
  function scrollTo(_x, _y) {
    ...
    if (!_element) return;
    (_c = _element instanceof Document ? window$1.document.body : _element) ==
      null ||
      _c.scrollTo({
  ...
  const setArrivedState = (target) => {
    ...
    if (target === window$1.document && !scrollTop)
      scrollTop = window$1.document.body.scrollTop;
...
function useLocalStorage(key, initialValue, options = {}) {
  ...
  return useStorage(
    key,
    initialValue,
    window$1 == null ? void 0 : window$1.localStorage,
    options,
  );
}
...
function useMagicKeys(options = {}) {
  ...
  function updateRefs(e, value) {
    ...
    const values = [
      (_b = e.code) == null ? void 0 : _b.toLowerCase(),
      key,
    ].filter(Boolean);
    if (key)
    ...
    get(target2, prop, rec) {
      ...
      if (!(prop in refs))
        if (/[+_-]/.test(prop)) {
          ...
          refs[prop] = computed(() =>
            keys$1.map((key) => toValue$1(proxy[key])).every(Boolean),
          );
...
function useMediaControls(target, options = {}) {
  ...
  const supportsPictureInPicture =
    document$1 && "pictureInPictureEnabled" in document$1;
  ...
  useEventListener(
    ...
    () =>
      ignoreCurrentTimeUpdates(
        () => (currentTime.value = toValue$1(target).currentTime),
      ),
    listenerOptions,
  );
  useEventListener(
...
function useMouseInElement(target, options = {}) {
  const { handleOutside = true, window: window$1 = defaultWindow } = options;
    ...
    stop = watch(
      ...
      () => {
        ...
        elementPositionX.value =
          left + (type === "page" ? window$1.pageXOffset : 0);
        elementPositionY.value =
          top + (type === "page" ? window$1.pageYOffset : 0);
...
function useNavigatorLanguage(options = {}) {
  const { window: window$1 = defaultWindow } = options;
  const navigator$1 = window$1 == null ? void 0 : window$1.navigator;
  const isSupported = useSupported(
    () => navigator$1 && "language" in navigator$1,
  );
  const language = shallowRef(
    navigator$1 == null ? void 0 : navigator$1.language,
  );
  useEventListener(
    window$1,
    "languagechange",
...
function useNetwork(options = {}) {
  ...
  const navigator$1 = window$1 == null ? void 0 : window$1.navigator;
  const isSupported = useSupported(
    () => navigator$1 && "connection" in navigator$1,
  );
...
function useClamp(value, min, max) {
  if (typeof value === "function" || isReadonly(value))
    return computed(() =>
      clamp(toValue$1(value), toValue$1(min), toValue$1(max)),
    );
...
function useOffsetPagination(options) {
  ...
  const pageCount = computed(() =>
    Math.max(1, Math.ceil(toValue$1(total) / toValue$1(currentPageSize))),
  );
...
function useParallax(target, options = {}) {
  ...
  const screenOrientation = reactive(
    useScreenOrientation({ window: window$1 }),
  );
  const {
    elementX: x,
    elementY: y,
...
function usePerformanceObserver(options, callback) {
  ...
  const isSupported = useSupported(
    () => window$1 && "PerformanceObserver" in window$1,
  );
  let observer;
  const stop = () => {
    observer?.disconnect();
...
function usePointerLock(target, options = {}) {
  const { document: document$1 = defaultDocument } = options;
  const isSupported = useSupported(
    () => document$1 && "pointerLockElement" in document$1,
  );
    ...
    useEventListener(
      ...
      () => {
        ...
        const currentElement =
          (_a = document$1.pointerLockElement) != null ? _a : element.value;
...
function usePreferredLanguages(options = {}) {
  const { window: window$1 = defaultWindow } = options;
  if (!window$1) return ref(["en"]);
  const navigator$1 = window$1.navigator;
  const value = ref(navigator$1.languages);
  useEventListener(
    window$1,
    "languagechange",
    () => {
...
function useScriptTag(src, onLoaded = noop, options = {}) {
  ...
  const loadScript = (waitForScriptLoad) =>
    new Promise((resolve, reject) => {
      ...
      let el = document$1.querySelector(`script[src="${toValue$1(src)}"]`);
      if (!el) {
  ...
  const unload = () => {
    ...
    const el = document$1.querySelector(`script[src="${toValue$1(src)}"]`);
...
function useSessionStorage(key, initialValue, options = {}) {
  ...
  return useStorage(
    key,
    initialValue,
    window$1 == null ? void 0 : window$1.sessionStorage,
    options,
  );
}
...
function useSpeechRecognition(options = {}) {
  ...
  const SpeechRecognition =
    window$1 &&
    (window$1.SpeechRecognition || window$1.webkitSpeechRecognition);
...
function useStyleTag(css, options = {}) {
  ...
  const load = () => {
    if (!document$1) return;
    const el =
      document$1.getElementById(id) || document$1.createElement("style");
    if (!el.isConnected) {
      el.id = id;
...
function useTextDirection(options = {}) {
  ...
  function getValue$1() {
    var _a, _b;
    return (_b =
      (_a = document$1 == null ? void 0 : document$1.querySelector(selector)) ==
      null
        ? void 0
  ...
  if (observe && document$1)
    useMutationObserver(
      document$1.querySelector(selector),
      () => (dir.value = getValue$1()),
      { attributes: true },
    );
  return computed({
    get() {
    ...
    set(v) {
      ...
      if (!document$1) return;
      if (dir.value)
        (_a = document$1.querySelector(selector)) == null ||
          _a.setAttribute("dir", dir.value);
      else
        (_b = document$1.querySelector(selector)) == null ||
...
function useTextSelection(options = {}) {
  ...
  if (window$1)
    useEventListener(window$1.document, "selectionchange", onSelectionChange, {
      passive: true,
    });
  return {
    text,
...
function useTimeAgo(time, options = {}) {
  ...
  const timeAgo = computed(() =>
    formatTimeAgo(new Date(toValue$1(time)), options, toValue$1(now$1)),
  );
  if (exposeControls)
    return {
      timeAgo,
...
function formatTimeAgo(from, options = {}, now$1 = Date.now()) {
  ...
  function getValue$1(diff2, unit) {
    return roundFn(Math.abs(diff2) / unit.value);
  }
  function format(diff2, unit) {
    const val = getValue$1(diff2, unit);
    const past = diff2 > 0;
  ...
  for (const [idx, unit] of units.entries()) {
    if (getValue$1(diff, unit) <= 0 && units[idx - 1])
...
function useTitle(newTitle = null, options = {}) {
  ...
  const originalTitle =
    (_a = document$1 == null ? void 0 : document$1.title) != null ? _a : "";
  const title = toRef(
    (_b =
      newTitle != null
        ? newTitle
  ...
  function format(t) {
    ...
    return typeof template === "function"
      ? template(t)
      : toValue$1(template).replace(/%s/g, t);
  }
  watch(
    title,
    (newValue, oldValue) => {
      if (newValue !== oldValue && document$1)
        document$1.title = format(newValue != null ? newValue : "");
    },
    { immediate: true },
  );
  if (options.observe && !options.titleTemplate && document$1 && !isReadonly$1)
    useMutationObserver(
      (_c = document$1.head) == null ? void 0 : _c.querySelector("title"),
      () => {
        if (document$1 && document$1.title !== title.value)
          title.value = format(document$1.title);
      },
      { childList: true },
    );
  tryOnScopeDispose(() => {
...
function executeTransition(source, from, to, options = {}) {
  ...
  const trans =
    typeof options.transition === "function"
      ? options.transition
      : (_b = toValue$1(options.transition)) != null
        ? _b
        : identity;
  const ease =
  ...
  return new Promise((resolve) => {
    ...
    const tick = () => {
      ...
      const alpha = ease((now$1 - startedAt) / duration);
      ...
      if (now$1 < endAt) requestAnimationFrame(tick);
      else {
        source.value = toVal;
        resolve();
      }
...
function useTransition(source, options = {}) {
  ...
  return computed(() =>
    toValue$1(options.disabled) ? sourceVal() : outputRef.value,
  );
}
function useUrlSearchParams(mode = "history", options = {}) {
  ...
  if (mode !== "history")
    useEventListener(window$1, "hashchange", onChanged, listenerOptions);
  const initial = read();
...
function useVibrate(options) {
  ...
  const vibrate = (pattern2 = patternRef.value) => {
    if (isSupported.value) navigator$1.vibrate(pattern2);
  };
  const stop = () => {
...
function useWakeLock(options = {}) {
  ...
  const isSupported = useSupported(
    () => navigator$1 && "wakeLock" in navigator$1,
  );
  ...
  if (isSupported.value) {
    ...
    whenever(
      () =>
        ...
        (document$1 == null ? void 0 : document$1.visibilityState) ===
          "visible" &&
        requestedType.value,
      (type) => {
  ...
  async function forceRequest(type) {
    ...
    sentinel.value = isSupported.value
      ? await navigator$1.wakeLock.request(type)
      : null;
  }
        ...
        if (
          (typeof retries === "function"
            ...
            : () =>
                ...
                (retries < 0 || retried < retries))(retried)
        ) {
          retried += 1;
...
function useWebWorker(arg0, workerOptions, options) {
  const { window: window$1 = defaultWindow } = options != null ? options : {};
  const data = ref(null);
