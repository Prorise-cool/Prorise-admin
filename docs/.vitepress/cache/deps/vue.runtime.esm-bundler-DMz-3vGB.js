...
var EffectScope = class {
  ...
  pause() {
    if (this._active) {
      ...
      let i, l;
      if (this.scopes)
        for (i = 0, l = this.scopes.length; i < l; i++) this.scopes[i].pause();
...
function setDevtoolsHook$1(hook, target) {
  ...
  if (devtools$1) {
    ...
    (target.__VUE_DEVTOOLS_HOOK_REPLAY__ =
      target.__VUE_DEVTOOLS_HOOK_REPLAY__ || []).push((newHook) => {
...
var devtoolsPerfStart =
  /* @__PURE__ */ createDevtoolsPerformanceHook("perf:start");
...
function getTransitionRawChildren(children, keepComment = false, parentKey) {
  ...
  if (keyedFragmentCount > 1)
    for (let i = 0; i < ret.length; i++) ret[i].patchFlag = -2;
  return ret;
}
/* @__NO_SIDE_EFFECTS__ */
...
function createSlots(slots, dynamicSlots) {
  for (let i = 0; i < dynamicSlots.length; i++) {
    ...
    if (isArray(slot))
      for (let j = 0; j < slot.length; j++) slots[slot[j].name] = slot[j].fn;
    else if (slot)
...
var publicPropertiesMap = /* @__PURE__ */ extend(
...
function normalizePropsOrEmits(props) {
  return isArray(props)
    ? props.reduce(
        (normalized, p$1) => ((normalized[p$1] = null), normalized),
        {},
      )
    : props;
}
...
function applyOptions(instance) {
  ...
  const checkDuplicateProperties = createDuplicateChecker();
  {
    ...
    if (propsOptions)
      for (const key in propsOptions) checkDuplicateProperties("Props", key);
  }
  if (injectOptions)
...
function callHook$1(hook, instance, type) {
  callWithAsyncErrorHandling(
    isArray(hook)
      ? hook.map((h$1) => h$1.bind(instance.proxy))
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const patch = (
    ...
    if (ref$1 != null && parentComponent)
      setRef(ref$1, n1 && n1.ref, parentSuspense, n2 || n1, !n2);
    else if (ref$1 == null && n1 && n1.ref != null)
  ...
  const setupRenderEffect = (
    ...
    toggleRecurse(instance, true);
    effect$1.onTrack = instance.rtc
      ? (e) => invokeArrayFns(instance.rtc, e)
      : void 0;
    effect$1.onTrigger = instance.rtg
  ...
  if (createHydrationFns)
    [hydrate$1, hydrateNode] = createHydrationFns(internals);
  return {
    render: render$1,
    hydrate: hydrate$1,
...
function emit(instance, event, ...rawArgs) {
  ...
  const modifiers =
    isModelListener$1 && getModelModifiers(props, event.slice(7));
  if (modifiers) {
    ...
    return (el, done) => {
      ...
      nextFrame(() => {
        ...
        if (!hasExplicitCallback(hook))
          whenTransitionEnds(el, type, enterDuration, resolve$1);
      });
    };
  };
    ...
    onLeave(el, done) {
      ...
      nextFrame(() => {
        ...
        if (!hasExplicitCallback(onLeave))
          whenTransitionEnds(el, type, leaveDuration, resolve$1);
      });
...
function patchAttr(
  ...
  isSVG,
  instance,
  isBoolean$1 = isSpecialBooleanAttr(key),
) {
  if (isSVG && key.startsWith("xlink:"))
    if (value == null) el.removeAttributeNS(xlinkNS, key.slice(6, key.length));
    else el.setAttributeNS(xlinkNS, key, value);
  else if (value == null || (isBoolean$1 && !includeBooleanAttr(value)))
    el.removeAttribute(key);
  else
    el.setAttribute(
      key,
      isBoolean$1 ? "" : isSymbol(value) ? String(value) : value,
    );
}
...
var VueElement = class VueElement extends BaseClass {
  ...
  _mount(def$1) {
    ...
    for (const key in exposed)
      if (!hasOwn(this, key))
        Object.defineProperty(this, key, { get: () => unref(exposed[key]) });
...
function callModelHook(el, binding, vnode, prevVNode, hook) {
  const fn = resolveDynamicModel(el.tagName, vnode.props && vnode.props.type)[
    hook
  ];
...
//# sourceMappingURL=vue.runtime.esm-bundler-DMz-3vGB.js.map
